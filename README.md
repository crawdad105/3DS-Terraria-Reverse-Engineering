...
but its extremally laggy and consumes multiple GB of ram in the browser,
it probably wont be updated often (or again).<br/>
...
[types.h](types.h) are the exported types form Binary Ninja as a header file.
The types were manually created, so they are incomplete
Its mainly used for referencing.<br/>
...
It generates [data.js](data.js), [userDefinedData.js](userDefinedData.js), [markdownFunctions.md](markdownFunctions.md), [markdownVariables.md](markdownVariables.md) and [extracted.txt](extracted.txt).
...
The old Codeglue mobile versions are the best option however as many functions are 1-to-1 with the 3DS.
...
Binary ninja files are upwards of a GB so they cant easily be shared,
also it would be distributing code which is illegal.<br/>
This can be parsed using [ExtractDumpNames.exe](ExportExtractor/ExtractDumpNames.exe) (you also need the other files as well).
...
The code might be compressed, it should be around 5KB, if its not run `ctrtool.exe --exefs=exefs.bin contents.0000.XXXXXXXX` and `ctrtool.exe --exefsdir=ExeFS --decompresscode exefs.bin`, this should uncompressed it.<br/>
...
[OTX tool](https://gbatemp.net/threads/otx-tool-terraria-texture-tool.458493/) can be used to extract image data form OTX files.

If you do not want to dump the cartage but want the code, you can dump the ram (which is better anyways since it gives more to work with) either using Rosalina or some other way.
...
These are the important sections/segments, there is an attached image of all of them.<br/>
...
See [markdownFunctions.md](markdownFunctions.md) (All exported symbols can be found in [extracted.txt](extracted.txt))
